...
export async function fetchCourseParticipants(courseId) {
  const { data, error } = await supabase
    .from("registrations")
.select("id, status, submitter_email, seats_held, session_id, theme_name, is_imported, import_seat_mode, created_at, advisors(full_name,phone), participants(id,full_name,school,grade_level,phone,email,national_id,participant_code,checkins(id,scanned_at))")    .order("created_at", { ascending: true })
  if (error) throw error
  return data || []
}
