...
export async function fetchCourses(eventId) {
  let q = supabase
    .from("courses")
    .select(
      "id, title, description, content, count_mode, team_size, min_members, max_members, capacity, seats_taken, price, bank_account, image_url, image_urls, level, start_date, end_date, duration, timeline, sessions, line_qr_url, form_schema, is_open, external_url, course_types:type_id(code,label,requires_payment,requires_approval,color), course_instructors(instructors(full_name)), course_days(day_date,start_at,end_at)"
    )
...
export async function fetchCourse(courseId) {
  const { data, error } = await supabase
    .from("courses")
    .select(
      "id, event_id, title, description, content, count_mode, team_size, min_members, max_members, capacity, seat_mode, require_portfolio, portfolio_label, seats_taken, price, bank_account, bank_name, bank_holder, line_qr_url, image_url, image_urls, level, start_date, end_date, duration, timeline, sessions, form_schema, is_open, external_url, course_types:type_id(code,label,requires_payment,requires_approval,color)"
    )
    .eq("id", courseId)
    .single()
  if (error) throw error
...
export async function fetchRegistrations(eventId) {
  let q = supabase
    .from("registrations")
    .select(
      "id, status, submitter_email, submitter_phone, seats_held, qr_token, waitlist_pos, created_at, course_id, theme_name, payment_deadline, courses!inner(title, event_id, price, require_portfolio), advisors(id,full_name,phone,email), participants(id,full_name,school,grade_level,phone,email,participant_code,qr_token,checkins(id,scanned_at)), payments(id,amount,slip_url,status)"
    )
...
export async function fetchCoursesAdmin(eventId) {
  let q = supabase
    .from("courses")
    .select("id, event_id, type_id, title, description, content, count_mode, team_size, min_members, max_members, capacity, seat_mode, require_portfolio, portfolio_label, seats_taken, price, bank_account, bank_name, bank_holder, image_url, image_urls, line_qr_url, base_id, level, start_date, end_date, duration, timeline, sessions, form_schema, is_open, external_url, course_types:type_id(label,color), course_instructors(instructors(full_name)), course_days(day_date)")
    .order("created_at", { ascending: true })
  if (eventId) q = q.eq("event_id", eventId)
  const { data, error } = await q
  if (error) throw error
...
export async function duplicateCourses(courseIds, toEventId) {
  ...
  const { data: sources, error: e1 } = await supabase
    .from("courses")
    .select("type_id, title, description, content, count_mode, team_size, min_members, max_members, capacity, seat_mode, require_portfolio, portfolio_label, price, bank_account, bank_name, bank_holder, image_url, image_urls, line_qr_url, base_id, level, start_date, end_date, duration, timeline, sessions, form_schema")
    .in("id", courseIds)
  if (e1) throw e1
