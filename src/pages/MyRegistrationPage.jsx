      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4 sm:py-5">
          ...
          <div className="relative mb-3">
            <Ico.search className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
            <input
              type="text"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder={t("myreg.searchPlaceholder") || "ค้นหาชื่อวิชา, หมวดหมู่..."}
              className="w-full bg-gray-50 border border-gray-200 rounded-xl py-2.5 pl-10 pr-10 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#F15A24]/40 focus:border-[#F15A24] focus:bg-white transition"
            />
            {search && (
              <button onClick={() => setSearch("")} aria-label="ล้างคำค้นหา"
                className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 flex items-center justify-center rounded-full bg-gray-200 text-gray-500 hover:bg-gray-300 transition text-xs leading-none">×</button>
            )}
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 mt-5 sm:mt-6">
        ...
        {regs.length === 0 ? (
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            <p className="font-semibold">{q ? "ไม่พบรายการที่ค้นหา" : t("myreg.noneInTab")}</p>
            {q && <p className="text-xs mt-1">ลองเปลี่ยนคำค้นหา "{search}"</p>}
          </div>
        ) : (
                ...
                <div key={reg.id} onClick={() => setDetailReg(reg)}
                  ...
                  <div className="p-4 sm:p-5 flex flex-col flex-1">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      {/* ───── ฝั่งซ้าย: วันที่ → ชื่อวิชา → หมวดหมู่ ───── */}
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-gray-400">
                          {t("myreg.registeredOn")} {fmtDate(reg.created_at)}
                        </p>

                        <h3 className="font-extrabold text-base sm:text-lg text-gray-800 leading-snug flex items-start gap-2 flex-wrap mt-1 mb-1.5">
                      ...
                      {/* ───── ฝั่งขวา: สถานะ(บน) + เงิน(ล่าง) ───── */}
                      <div className="flex flex-col items-end gap-2 shrink-0">
                        <span className={`inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1 rounded-full border whitespace-nowrap ${cfg.bg} ${cfg.text} ${cfg.border}`}>
                          {cfg.icon} {t(cfg.key)}
