    ...
    <div>
      {!embedded && (
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-5">
          ...
          <button onClick={() => setModal({ name: "CAMT Summer Course", year: new Date().getFullYear() + 543, status: "draft" })}
            ...
            <Ico.plus className="w-4 h-4" /> สร้างงานปีใหม่
          </button>
        </div>
      )}
      ...
      {!embedded && (
        <div className="bg-gradient-to-r from-[#fff5f0] to-white border border-orange-100 rounded-2xl p-4 mb-5 flex items-center gap-3">
          <Ico.megaphone className="w-6 h-6 text-[#F15A24] shrink-0" />
              ...
              <div key={ev.id} className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 sm:p-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 shrink-0">
                    ...
                    <button onClick={() => setModal(ev)} className="bg-yellow-50 text-yellow-700 border border-yellow-200 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-yellow-100 transition inline-flex items-center gap-1.5"><Ico.pencil className="w-3.5 h-3.5" /> แก้ไข</button>
                    <button onClick={() => doDelete(ev)} className="bg-red-50 text-red-600 border border-red-200 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-red-100 transition inline-flex items-center"><Ico.trash className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
              </div>
            )
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4" onClick={onClose}>
      <div className="bg-white w-full sm:rounded-2xl sm:max-w-md shadow-2xl overflow-hidden rounded-t-2xl" onClick={(e) => e.stopPropagation()}>
        <div className={`${isEdit ? "bg-yellow-500" : "bg-[#F15A24]"} px-5 py-4 flex justify-between items-center`}>
          <h3 className="font-bold text-white text-base flex items-center gap-2">{isEdit ? <><Ico.pencil className="w-4 h-4" /> แก้ไขงาน</> : <><Ico.plus className="w-4 h-4" /> สร้างงานปีใหม่</>}</h3>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400 mt-1 flex items-start gap-1"><Ico.bulb className="w-3.5 h-3.5 shrink-0 mt-px" /> ตั้ง "เปิดรับสมัคร" ได้ทีละงาน — งานอื่นที่เปิดอยู่จะถูกปิดให้</p>
          </div>
        </div>
        <div className="px-5 pb-5 grid grid-cols-2 gap-3">
          ...
          <button onClick={() => onSave(f)} className={`py-3 text-white rounded-xl font-bold shadow-sm transition text-sm ${isEdit ? "bg-yellow-500 hover:bg-yellow-600" : "bg-[#F15A24] hover:bg-orange-600"}`}>
            <span className="inline-flex items-center justify-center gap-1.5">{isEdit ? <><Ico.save className="w-4 h-4" /> บันทึก</> : <><Ico.check className="w-4 h-4" /> สร้างงาน</>}</span>
          </button>
        </div>
      </div>
    </div>
